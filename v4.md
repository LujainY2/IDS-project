...
def add_predictions(input_data):
    st.subheader("Attack Predictions:")
    try:
...
def get_analysis_data():
    ...
    for path in file_paths:
        try:
            ...
            df = df[df["Protocol"] != "Protocol"]  # Remove malformed header rows
            df = df[df["Label"] != "Label"]
            dataset_list.append(df)
        except Exception as e:
    ...
    if label_counts.empty:
        st.warning("No valid labels found in the data.")
        return df

    # Sampling balanced benign vs malicious
    df_benign = df[df['Label'] == 'Benign']
    df_malicious = df[df['Label'] != 'Benign']

    # Choose to keep all benign and sample malicious with a fraction (e.g. 50%)
    malicious_sample_fraction = 0.5  # Adjust as needed
    df_malicious_sampled = df_malicious.sample(frac=malicious_sample_fraction, random_state=42)

    df_sampled = pd.concat([df_benign, df_malicious_sampled]).sample(frac=1, random_state=42).reset_index(drop=True)
    return df_sampled
...
if st.session_state.page == "Main":
    ...
    image_path = "streamlit_app/images/AI-Cybersecurity - Edited.png"
    ...
    if predicted_class:
        ...
        # Call prediction function first to get the result
        chart_colors = [
    ...
    # Define protocol and port-service mappings
    protocol_map = {6: 'TCP', 17: 'UDP', 1: 'ICMP', 2: 'IGMP', 0: 'HOPOPT'}
    ...
    protocol_distribution = df['Protocol_Name'].value_counts().reset_index()
    protocol_distribution.columns = ['Protocol', 'Count']

    top_services = df['Service'].value_counts().nlargest(10).reset_index()
    ...
    with col2:
        st.metric("🚨 Malicious Attacks", malicious_count)

    st.subheader("🧪 Label Distribution")
    fig2 = px.bar(label_distribution, x='Label', y='Count', color='Label')
    st.plotly_chart(fig2, use_container_width=True)

    st.subheader("📡 Protocol Distribution")
    fig3 = px.pie(protocol_distribution, names='Protocol', values='Count')
    st.plotly_chart(fig3, use_container_width=True)

    st.subheader("🌐 Top Destination Port Services")
    fig4 = px.bar(top_services, x='Service', y='Count', color='Service')
    st.plotly_chart(fig4, use_container_width=True)

    # MALICIOUS TRAFFIC ANALYSIS
    st.subheader("🔍 Malicious Traffic Analysis")
    threshold = 100000
    benign_value = 'Benign'
    ...
    # DONUT CHARTS FOR INSIGHT
    st.subheader("📊 Network Traffic Insights")
    col1, col2, col3 = st.columns(3)

    with col1:
        fig6 = px.pie(protocol_distribution, names='Protocol', values='Count', hole=0.4,
                      title="🧹 Protocol Distribution")
        st.plotly_chart(fig6, use_container_width=True)

    with col2:
        attack_distribution = df[df['Label'] != 'Benign']['Label'].value_counts().reset_index()
        attack_distribution.columns = ['Attack Type', 'Count']
        fig7 = px.pie(attack_distribution, names='Attack Type', values='Count', hole=0.4,
                      title="🔥 Attack Type Breakdown")
        st.plotly_chart(fig7, use_container_width=True)

    with col3:
        fig8 = px.pie(top_services, names='Service', values='Count', hole=0.4,
                      title="🌐 Service Distribution")
        st.plotly_chart(fig8, use_container_width=True)

    df_malicious = df[df['Label'] != 'Benign']
    malicious_unique = df_malicious.drop_duplicates(subset='Label', keep='first')
    st.subheader("🧾 Unique Malicious Traffic Types")
    st.dataframe(malicious_unique[['Timestamp', 'Label', 'Protocol', 'Dst Port', 'Flow Pkts/s']],
    ...
    fig9 = px.scatter(normal_data, x='Timestamp', y='Flow Pkts/s',
                      title="📊 Flow Pkts/s with Highlighted Anomalies",
                      labels={"Flow Pkts/s": "Flow Packets per Second"}, height=500)
    fig9.update_traces(marker=dict(color='blue', size=6), name="Normal")
